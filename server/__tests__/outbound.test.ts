...
describe("resolveOutboundDestination", () => {
  it("resolves featured ticket redirects", async () => {
    ...
    expect(resolveOutboundDestination("tickets", "featured")).toMatch(
      /^https:\/\//
    );
  });

  it("resolves event-specific ticket redirects with explicit mappings", async () => {
    process.env.OUTBOUND_TICKETS_US_S3E3_URL =
  ...
  it("fails closed for Sun(Sets) ticket rails until the official Posh URL is configured", async () => {
    ...
    const { resolveOutboundDestination, TICKETS_COMING_SOON } =
      await importOutbound();
  ...
  it("resolves Sun(Sets) tickets only from their official event env vars", async () => {
    ...
    process.env.OUTBOUND_TICKETS_CSS_SEP19_URL =
  ...
  it("resolves named waitlist redirects", async () => {
    const { resolveOutboundDestination } = await importOutbound();
    expect(resolveOutboundDestination("waitlist", "untold-story")).toMatch(
      /^https:\/\//
    );
    expect(
  ...
  it("lets OUTBOUND_LAYLO_URL drive the general and Chasing Sunsets waitlist rail", async () => {
    process.env.OUTBOUND_LAYLO_URL = "https://laylo.example.com/drop/sunsets";
    delete process.env.OUTBOUND_WAITLIST_GENERAL_URL;
    delete process.env.OUTBOUND_WAITLIST_CHASING_SUNSETS_URL;
    delete process.env.OUTBOUND_LAYLO_SUNSETS_URL;

    const { resolveOutboundDestination } = await importOutbound();

    expect(resolveOutboundDestination("waitlist", "general")).toBe(
      "https://laylo.example.com/drop/sunsets"
    );
    expect(resolveOutboundDestination("waitlist", "chasing-sunsets")).toBe(
      "https://laylo.example.com/drop/sunsets"
    );
  });
  ...
  it("resolves Sunsets media, gallery, form, and social redirects", async () => {
    ...
    expect(resolveOutboundDestination("media", "sunsets-recap")).toContain(
      "youtu"
    );
    expect(resolveOutboundDestination("media", "sunsets-soundcloud")).toContain(
      "soundcloud.com"
    );
  ...
  it("locks Sun(Sets) July 4 outbound Posh UTMs while preserving click IDs", async () => {
    ...
    const destination = decorateOutboundDestination(
      "https://posh.vip/e/sunsets-july-4?foo=bar",
      {
        session_id: "sess_123",
  ...
  it("preserves UTM and click-id params on outbound redirects", async () => {
    const { decorateOutboundDestination } = await importOutbound();
    const destination = decorateOutboundDestination(
      "https://tickets.example.com/event?utm_source=posh",
      {
        session_id: "sess_123",
