...
function buildLayloLead(payload: Record<string, unknown>) {
  ...
  const dropName =
    pickString(payload, ["drop.name", "dropName", "drop_name", "campaign.name"]) ||
    SUNSETS_JULY4_EVENT_TITLE;
  const dropSlug =
    pickString(payload, ["drop.slug", "dropSlug", "drop_slug", "campaign.slug"]) ||
    SUNSETS_JULY4_EVENT_SLUG;
  const signupChannel =
    pickString(payload, ["source", "channel", "signupChannel", "signup_channel", "utm_source"]) ||
    "laylo_webhook";
  ...
  return {
    ...
    dropSlug,
    signupChannel,
    utmSource: normalizedOptional(pickString(payload, ["utm_source", "utm.source", "utmSource"])),
    utmMedium: normalizedOptional(pickString(payload, ["utm_medium", "utm.medium", "utmMedium"])),
    utmCampaign: normalizedOptional(pickString(payload, ["utm_campaign", "utm.campaign", "utmCampaign"])),
    utmContent: normalizedOptional(pickString(payload, ["utm_content", "utm.content", "utmContent"])),
    ref: normalizedOptional(pickString(payload, ["ref", "reference", "referrer"])),
  };
}
