...
const sponsorDeckLimiter = createRateLimitMiddleware({
  ...
  limit: 20,
  message:
    "Too many document requests. Please wait 15 minutes before retrying.",
});
...
router.post("/api/sponsor-access", sponsorAccessLimiter, (req, res) => {
  ...
  res.setHeader(
    ...
    buildSponsorSessionCookie(
      sessionToken,
      Math.floor(sponsorSessionTtlMs / 1000)
    )
  );
  ...
  return res.status(200).json({
    ok: true,
    requestId,
    sessionExpiresInSec: Math.floor(sponsorSessionTtlMs / 1000),
  });
});

router.get("/api/sponsor-deck", sponsorDeckLimiter, (req, res) => {
  const requestId = randomUUID();
  const sessionToken = parseCookieHeader(req.header("cookie"))[
    sponsorSessionCookieName
  ];
  ...
  res.setHeader(
    "Set-Cookie",
    buildSponsorSessionCookie(newToken, Math.floor(sponsorSessionTtlMs / 1000))
  );
  ...
  return res.download(sponsorDeckPath, sponsorDeckFilename, error => {
    ...
    logEvent("sponsor.deck_download_failed", {
      requestId,
      code,
    ...
    return res.status(isMissing ? 404 : 500).json({
      ...
      error: {
        ...
        message: isMissing
          ? "Sponsor deck is unavailable."
          : "Unable to download sponsor deck right now.",
        retryable: !isMissing,
      },
    });
  });
