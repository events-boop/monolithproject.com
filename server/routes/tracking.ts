...
const LAKE_PIXEL_ID =
  process.env.META_LAKE_PIXEL_ID?.trim() || "1049241148606250";
...
const trackingLimiter = createRateLimitMiddleware({
  ...
  limit: 240,
  message:
    "Too many tracking events. Please wait 15 minutes before trying again.",
});
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (db) {
      await db
        .insert(funnelPageViews)
        .values({
          id: randomUUID(),
          contactId: parsed.data.contactId || null,
          anonymousSessionId:
            parsed.data.anonymousSessionId || parsed.data.sessionId || null,
          pagePath: parsed.data.pagePath,
          eventSlug: parsed.data.eventSlug || null,
          source: parsed.data.source || null,
          utmSource:
            parsed.data.utmSource ||
            parsed.data.lastUtmSource ||
            parsed.data.firstUtmSource ||
            null,
          utmMedium:
            parsed.data.utmMedium ||
            parsed.data.lastUtmMedium ||
            parsed.data.firstUtmMedium ||
            null,
          utmCampaign:
            parsed.data.utmCampaign ||
            parsed.data.lastUtmCampaign ||
            parsed.data.firstUtmCampaign ||
            null,
          utmContent:
            parsed.data.utmContent ||
            parsed.data.lastUtmContent ||
            parsed.data.firstUtmContent ||
            null,
          utmTerm:
            parsed.data.utmTerm ||
            parsed.data.lastUtmTerm ||
            parsed.data.firstUtmTerm ||
            null,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const anonymousSessionId =
      data.anonymousSessionId || data.sessionId || null;
    const utmSource =
      data.utmSource || data.lastUtmSource || data.firstUtmSource || null;
    const utmCampaign =
      data.utmCampaign || data.lastUtmCampaign || data.firstUtmCampaign || null;
    const db = getDatabase();
    ...
    if (db) {
      await db
        ...
        .values({
          ...
          utmSource,
          utmMedium:
            data.utmMedium || data.lastUtmMedium || data.firstUtmMedium || null,
          utmCampaign,
          utmContent:
            data.utmContent ||
            data.lastUtmContent ||
            data.firstUtmContent ||
            null,
          utmTerm:
            data.utmTerm || data.lastUtmTerm || data.firstUtmTerm || null,
          metadata: data,
        })
        .catch(error => {
      ...
      if (contentType) {
        await db
          ...
          .values({
            ...
            contentId:
              data.eventSlug ||
              data.buttonName.toLowerCase().replace(/\s+/g, "-"),
            contentType,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    logEvent("tracking.lead_conversion", {
      requestId,
      eventId,
      pixelId: LAKE_PIXEL_ID,
    });
