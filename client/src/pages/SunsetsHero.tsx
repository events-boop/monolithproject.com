    ...
    <>
      ...
      >
        ...
        <nav className="relative z-50 flex items-center justify-between px-6 py-4 border-b border-white/10 bg-black/50 backdrop-blur-md">
          <div className="text-3xl font-black tracking-tighter" style={{ fontFamily: 'Impact, sans-serif' }}>
            SUN(SETS)
          </div>
          <div className="hidden md:flex items-center gap-8 text-sm font-bold tracking-wider">
            ...
            <Link href="/" className="hover:text-pink-400 transition-colors">EVENTS</Link>
            <Link href="/" className="hover:text-pink-400 transition-colors">TICKETS</Link>
            <Link href="/" className="hover:text-pink-400 transition-colors">EXPERIENCE</Link>
            <Link href="/" className="hover:text-pink-400 transition-colors">GALLERY</Link>
            <Link href="/" className="hover:text-pink-400 transition-colors">INFO</Link>
          </div>
        ...
        <main className="relative z-10 container mx-auto px-6 pt-16 pb-32 flex flex-col lg:flex-row items-center justify-between gap-12">
          ...
          <div className="flex-1 flex flex-col items-start text-left max-w-2xl">
            ...
            <p className="text-lg md:text-xl font-medium leading-relaxed max-w-lg mb-10 text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
              Chasing Sun(Sets) returns to the lake for Independence Day. A full-day Chicago lakefront celebration with house music, fireworks, skyline views, and good people.
            </p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 w-full max-w-xl">
              {[
                ...
                <div key={i} className="flex flex-col items-center text-center gap-2">
                  <div className={`w-16 h-16 rounded-full border-2 border-current ${badge.color} bg-black/60 flex items-center justify-center text-3xl shadow-lg backdrop-blur-sm`}>
                    {badge.icon}
                  </div>
                  <div className={`text-[10px] font-bold tracking-widest whitespace-pre-line leading-tight ${badge.color} drop-shadow-md`}>
                    {badge.label}
                  </div>
                </div>
              ))}
            ...
            <div className="relative w-full aspect-[4/5] bg-[#111] p-4 rounded-lg shadow-[0_30px_60px_-15px_rgba(0,0,0,0.9)] transform md:rotate-2 hover:rotate-0 transition-all duration-500 border border-gray-800">
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black via-black/80 to-transparent p-6 pb-12 text-center border-t-2 border-pink-500/50">
                  ...
                  <div className="text-3xl sm:text-4xl font-black text-white tracking-tighter leading-none mb-2" style={{ fontFamily: 'Impact, sans-serif' }}>
                    AUTOGRAF <span className="text-pink-500">★</span> KIKO FRANCO
                  </div>
                  <div className="text-sm sm:text-base font-bold text-yellow-400 tracking-widest uppercase">
                    AMARI • ERIK THE DJ
                  </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-black border-t border-yellow-400/30 overflow-hidden py-3 z-[9999]">
          <div className="flex whitespace-nowrap" style={{ animation: 'marquee 25s linear infinite' }}>
            ...
            {[1, 2, 3, 4, 5, 6, 7, 8].map((_, i) => (
              <div key={i} className="flex items-center gap-6 text-yellow-400 font-bold tracking-widest text-sm md:text-base uppercase px-6 shrink-0">
