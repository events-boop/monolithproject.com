...
export default function AdminDashboard() {
  ...
  const runFullAudit = () => {
    ...
    staleEvents.forEach(e => {
      results.push({
        ...
        suggestion:
          "Remove from featured events or update status to 'past' in public-site-data.ts.",
      });
    });
    ...
    setTimeout(() => {
      setConnectors([
        {
          name: "Posh.vip (Tickets)",
          status: publicEvents.some(event => Boolean(event.ticketUrl))
            ? "active"
            : "error",
          latency: "42ms",
        },
        { name: "Laylo (Access)", status: "active", latency: "88ms" },
        {
          ...
          status:
            typeof (window as any).gtag !== "undefined" ? "active" : "error",
          latency: "12ms",
        },
  ...
  const handleInvalidateCache = async () => {
    setIsScanning(true);
    try {
      const response = await fetch("/api/ops/cache/invalidate", {
        method: "POST",
      });
      if (response.ok) {
      ...
      <div className="min-h-screen bg-[#050505] text-white flex flex-col items-center justify-center p-6 bg-scanlines relative">
        ...
        >
          ...
          <form onSubmit={handleAccessChallenge} className="space-y-6">
            ...
            <button type="submit" className="btn-pill-monolith btn-pill-wide">
              Authorize Entry
            </button>
          </form>
        </motion.div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-8 border-b border-white/10 pb-12">
            <div>
              <span className="font-mono text-[10px] tracking-[0.4em] uppercase text-primary mb-4 block">
                Command_Center v1.1 // AUTHORIZED
              </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-16">
            {[
              ...
              {
                label: "Status",
                val: issues.length === 0 ? "STABLE" : "DEGRADED",
                icon: CheckCircle,
                color:
                  issues.length === 0 ? "text-green-400" : "text-amber-400",
              },
              {
                label: "Audit Ghosts",
                val: issues.length,
                ...
                {[
                  ...
                  >
                    ...
                    <div className="flex items-end justify-between">
                      ...
                      <span className="font-mono text-[8px] text-green-500 bg-green-500/10 px-1.5 py-0.5 rounded leading-none">
                        TARGET
                      </span>
                    </div>
                  </div>
              ...
              <div className="space-y-4">
                {connectors.map((c, i) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-2 h-2 rounded-full ${c.status === "active" ? "bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]" : c.status === "error" ? "bg-red-500" : "bg-white/20 animate-pulse"}`}
                      />
            ...
            {issues.length > 0 ? (
              issues.map((issue, i) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`p-4 rounded-xl ${issue.level === "critical" ? "bg-red-500/10" : "bg-amber-500/10"}`}
                    >
                      <AlertTriangle
                        className={
                    ...
                    <div className="flex-1">
                      <span
                        className={`font-mono text-[10px] tracking-widest uppercase block mb-2 ${issue.level === "critical" ? "text-red-400" : "text-amber-400"}`}
                      >
                        {issue.type}
                      </span>
                      ...
                      <div className="flex flex-wrap items-center gap-4">
                        ...
                        <Link
                          ...
                          className="text-[10px] font-mono uppercase tracking-widest hover:text-primary transition-colors flex items-center gap-2"
                        >
