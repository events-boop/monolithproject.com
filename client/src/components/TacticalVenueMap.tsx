...
const VENUE_ZONES: Zone[] = [
  {
    ...
    label: "Main Floor",
    icon: Users,
    desc: "The spatial core. Kinetic density 100%. GA access point.",
    coord: { x: "50%", y: "60%" },
    type: "ga",
  },
  {
    ...
    label: "Signal Booth",
    icon: Music,
    desc: "Primary audio emitter. No-access exclusion zone.",
    coord: { x: "50%", y: "15%" },
    type: "peak",
  },
  {
    ...
    label: "VIP Balcony",
    icon: ShieldCheck,
    desc: "Elevated perspective. Restricted clearance required.",
    coord: { x: "20%", y: "45%" },
    type: "vip",
  },
  {
    id: "the-bar",
    label: "hydration node",
    icon: CupSoda,
    desc: "Liquid replenishment. Cashless terminal interface.",
    coord: { x: "85%", y: "50%" },
    type: "service",
  },
  {
    ...
    label: "check-in",
    icon: MapPin,
    desc: "Primary vector for environmental entry. Verify ID here.",
    coord: { x: "50%", y: "90%" },
    type: "service",
  },
];
        ...
        <div className="relative bg-[#050505] p-12 overflow-hidden flex items-center justify-center">
          ...
          <div className="w-full h-full border border-white/5 rounded-lg relative flex items-center justify-center">
            ...
            {VENUE_ZONES.map(zone => (
              ...
              >
                <div
                  className={`absolute inset-0 rounded-full animate-ping opacity-20 ${activeZone?.id === zone.id ? "bg-primary scale-150" : "bg-white/20"}`}
                />
                <div
                  className={`w-8 h-8 rounded-full border flex items-center justify-center backdrop-blur-md transition-colors ${activeZone?.id === zone.id ? "bg-primary border-primary text-black" : "bg-black/80 border-white/20 text-white group-hover:border-primary group-hover:text-primary"}`}
                >
                  <zone.icon size={14} />
                </div>
                <div className="absolute top-full mt-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                  <span className="text-[10px] bg-black border border-white/10 px-2 py-1 rounded text-white whitespace-nowrap tracking-widest">
                    {zone.label}
                  </span>
                </div>
              </motion.button>
            ...
            {activeZone ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-center opacity-20">
                ...
                <p className="text-[10px] uppercase tracking-[0.3em]">
                  Select a vector to initialize briefing.
                </p>
              </div>
            )}
          </AnimatePresence>
