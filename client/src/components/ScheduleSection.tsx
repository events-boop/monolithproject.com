...
export default function ScheduleSection() {
  ...
  const months = [
    "ALL",
    ...Array.from(
      new Set(upcomingEvents.map(e => e.date.split(" ")[0].toUpperCase()))
    ),
  ];
  ...
  const filteredEvents =
    activeMonth === "ALL"
      ...
      : upcomingEvents.filter(e =>
          e.date.toUpperCase().startsWith(activeMonth)
        );
          ...
          {filteredEvents.length === 0 ? (
            <div className="text-center py-20 md:py-32">
              ...
              <p className="font-mono text-[11px] font-bold uppercase tracking-widest text-white/60">
                No matching dates for {activeMonth}
              </p>
            </div>
          ) : (
            filteredEvents.map((event, index) => {
              ...
              const shouldPing =
                event.recentlyDropped || event.status === "on-sale";
              const detailsHref = getEventDetailsHref(event);
              const isJulyHoliday =
                dateMonth.toUpperCase().startsWith("JUL") &&
                (dayNumber === 4 || dayNumber === 5);
                    ...
                    <div className="py-6 md:py-10 flex flex-col lg:grid lg:grid-cols-12 gap-5 md:gap-6 lg:items-center w-full text-left">
                      ...
                      <div className="lg:col-span-2 flex items-center justify-between lg:justify-start lg:flex-col lg:items-start gap-2 lg:gap-1 pl-0 lg:pl-4">
                        <div className="flex flex-col">
                          ...
                          >
                            {dayNumber
                              ? `${dateMonth.substring(0, 3).toUpperCase()} ${dayNumber}`
                        ...
                        {shouldPing && (
                          <div className="flex items-center gap-2 rounded-full border border-white/10 bg-black/[0.055] px-2 py-1 lg:hidden">
                            <span className="relative flex h-2 w-2">
                              <span
                                className={`absolute inline-flex h-full w-full animate-ping rounded-full opacity-75 ${event.recentlyDropped ? "bg-cyan-500" : "bg-primary"}`}
                              />
                              <span
                                className={`relative inline-flex rounded-full h-2 w-2 ${event.recentlyDropped ? "bg-cyan-500" : "bg-primary"}`}
                              />
                            </span>
                        ...
                        {shouldPing && (
                          <div className="relative flex h-10 w-10 items-center justify-center overflow-hidden rounded-full border border-white/10 bg-black shadow-sm">
                            <motion.div
                              ...
                              style={{
                                backgroundColor: event.recentlyDropped
                                  ? "#06B6D4"
                      ...
                      <div className="lg:col-span-2 hidden lg:flex flex-col">
                        ...
                        <span className="mt-0.5 font-mono text-[10px] font-bold uppercase tracking-widest text-white/58">
                          {event.location}
                        </span>
                      </div>
                      ...
                      <div className="lg:col-span-3 flex flex-wrap items-center justify-between lg:justify-end gap-3 lg:pr-4 w-full mt-2 lg:mt-0">
                        <div className="flex-1 min-w-[11rem] lg:flex-none z-20">
                          <ConversionCTA
                            ...
                            size="sm"
                            showUrgency={true}
                            className="schedule-section-cta w-full md:w-auto"
                          />
                        </div>
                        <button
                          type="button"
                          ...
                          <div className="lg:col-span-4 order-2 lg:order-1">
                            <div className="relative aspect-video overflow-hidden rounded-2xl border border-white/15 shadow-[0_18px_45px_rgba(0,0,0,0.16)] lg:aspect-[4/5]">
                              ...
                              <div className="absolute bottom-4 left-4 flex flex-col">
                                <span className="hero-wordmark text-xs text-white uppercase tracking-tighter">
                                  {event.series.replace("-", " ")}
                                </span>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="lg:col-span-8 order-1 lg:order-2 flex flex-col justify-center">
                            ...
                            <p className="mb-10 max-w-3xl font-serif text-xl italic leading-snug text-white/86 md:mb-12 md:text-2xl lg:text-3xl">
                              {event.description ||
                                ...
                                "Join us for a night built on sound, intention, and proper scale."}
                            </p>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 mb-10 lg:mb-12">
                              ...
                              <div className="rounded-3xl border border-white/10 bg-black/[0.02] p-6 shadow-[0_14px_36px_rgba(0,0,0,0.07)] md:p-8">
                                ...
                                <p className="mt-3 font-mono text-[10px] font-bold uppercase tracking-widest text-white/62">
