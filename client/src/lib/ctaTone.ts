...
export type CtaTone = ScheduledEvent["series"] | "sunsets-radio" | "monolith";
...
export function getEventCtaToneClass(
  event?: { series?: ScheduledEvent["series"] } | null
) {
  return event?.series
    ? TONE_CLASS_BY_SERIES[event.series]
    : TONE_CLASS_BY_TONE.monolith;
}

export function getEventPillToneClass(
  event?: { series?: ScheduledEvent["series"] } | null
) {
  return event?.series
    ? PILL_CLASS_BY_SERIES[event.series]
    : "btn-pill-monolith";
}

export function getEventOutlinePillToneClass(
  event?: { series?: ScheduledEvent["series"] } | null
) {
  return event?.series
    ? OUTLINE_PILL_CLASS_BY_SERIES[event.series]
