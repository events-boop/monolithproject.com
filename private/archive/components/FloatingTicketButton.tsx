...
export default function FloatingTicketButton() {
  ...
  const accentColor = isSunsets
    ? "bg-[#E8B86D]"
    : isStory
      ? "bg-[#22D3EE]"
      : "bg-primary";
  const textAccent = isSunsets
    ? "text-[#E8B86D]"
    : isStory
      ? "text-[#22D3EE]"
      : "text-primary";
  ...
  const shortDate =
    dateParts.length >= 2
      ? `${dateParts[0].substring(0, 3)} ${dateParts[1].replace(",", "")}`
      : nextEvent.date;
    ...
    >
      ...
      >
        <motion.div
          ...
          className="bg-black border-t md:border border-white/15 p-5 sm:p-6 md:p-8 flex items-center justify-between md:justify-end gap-4 md:gap-8 transition-all duration-500 hover:bg-white hover:border-white overflow-hidden shadow-[0_-10px_40px_rgba(0,0,0,0.8)] md:shadow-2xl lg:hover:[filter:url(#liquid-distortion)]"
        >
          {/* Edge Accent Line */}
          <div
            className={`absolute top-0 bottom-0 left-0 w-[3px] opacity-50 group-hover:opacity-100 transition-all duration-500 ${accentColor}`}
          />

          <div className="flex flex-col text-left md:text-right items-start md:items-end z-10 w-full md:w-auto overflow-hidden">
            {nextEvent.ticketUrl && (
              <div className="flex items-center gap-2 mb-2 md:mb-3">
                <span className="relative flex h-2 w-2">
                  <span
                    className={`animate-ping absolute inline-flex h-full w-full opacity-75 ${accentColor}`}
                  />
                  <span
                    className={`relative inline-flex h-2 w-2 ${accentColor}`}
                  />
                </span>
                <span
                  className={`font-mono text-[11px] md:text-xs uppercase tracking-[0.35em] font-bold ${textAccent} group-hover:text-black/60 transition-colors`}
                >
                  Tickets Live
                </span>
              </div>
