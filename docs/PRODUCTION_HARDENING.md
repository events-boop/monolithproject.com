...
- **Global API Rate Limiting**: The `/api` endpoints are protected by a custom rate limiter from `server/services/rate-limit.ts` with a `2000 requests / 15 minutes` policy. It uses database-backed buckets when storage is available and falls back to in-memory buckets if persistence fails.
- **Webhook Pre-Parser Authentication**: Requests to the Posh webhook endpoint (`/api/webhooks/posh`) are validated prior to parsing the JSON payload. Missing or invalid `Posh-Secret` headers are rejected early with `401 Unauthorized` or `503 Service Unavailable` before Express JSON parsing/body-parser buffering occurs. The raw body buffer is preserved under `req.rawBody` for downstream validation checks.
- **Outbound Redirect Hardening (`/go/*`)**: Outbound redirection mappings use null-prototype objects and strict `hasOwnProperty` checks to ensure that query parameters matching prototype properties (e.g. `__proto__`, `toString`, `constructor`) are rejected and blocked (returning `404 Not Found`) rather than resolving to prototype methods or fallback destinations.
...
_Created for the Monolith Project July 4th Campaign Deployment._
破
